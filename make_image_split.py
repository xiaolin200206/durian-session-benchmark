...
The source partition holds eleven classes: the five disease classes the paper
reports plus six pest-damage classes added later. The pest classes are outside
the disease vocabulary under study and are NOT part of the released dataset;
this is stated in Sect. 3.2 of the paper. This script copies out only the five
disease classes, preserving the train/val/test assignment exactly as it was.
...
# The IMAGE-LEVEL control partition, for verification.
# NOTE: these are NOT the counts of Table 1. Table 1 reports the session-level
# partition (446/58/56). The image-level control below is 446/54/60: the
# training-set size is matched deliberately so that the two conditions differ
# in the grouping rule rather than in training-set size, at the cost of test
# sets of similar but not identical size (Sect. 3.4 of the paper).
EXPECTED = {
    "Algal":        (129, 16, 17),
    "Leaf_rot":     (122, 15, 16),
    "Phomopsis":    (125, 15, 17),
...
def main():
    ...
    hdr = f"{'class':<16}{'train':>7}{'val':>6}{'test':>6}{'total':>7}   vs image-level control"
    print(hdr)
    print("-" * len(hdr))
    ok = True
    for cls in PAPER_CLASSES:
    ...
    if total != 560 or not ok:
        print("\n!! Counts do not reproduce the image-level control (446/54/60). "
